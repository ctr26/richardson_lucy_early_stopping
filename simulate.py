...
Image.fromarray(rescale_intensity(T.reshape(astro.shape), out_range=np.uint8)).convert(
    "L"
).save(os.path.join(out_dir, "T.png"))
Image.fromarray(rescale_intensity(f.reshape(astro.shape), out_range=np.uint8)).convert(
    "L"
).save(os.path.join(out_dir, "f.png"))
Image.fromarray(rescale_intensity(V.reshape(astro.shape), out_range=np.uint8)).convert(
    "L"
).save(os.path.join(out_dir, "V.png"))
Image.fromarray(
    rescale_intensity(g_blurred.reshape(astro.shape), out_range=np.uint8)
).convert("L").save(os.path.join(out_dir, "g_blurred.png"))
Image.fromarray(rescale_intensity(astro, out_range=np.uint8)).convert("L").save(
